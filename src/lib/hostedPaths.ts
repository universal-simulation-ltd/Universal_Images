// Object paths for "Hosted by UNI·SIM" images — and the repair for the ones the
...
//   2. upload the bytes to `<org>/images/<upload_id>-<slug>.<ext>`
...
// EVERY hosted image's ledger row kept `storage_path = 'pending'`. The list in
...
//
//   * `hostedImagePath` names the object BEFORE the token is reserved, so the
//     ledger records the truth at insert time and step 3 disappears; and
//   * `hostedImagePathCandidates` rebuilds where a legacy row's bytes actually
//     went. The old path was fully determined by data still on the row —
//     `<org_id>/images/<id>-<safeName(file_name)>` — so a 'pending' row is
...
/** The `images` segment of every Universal Images object path. */
export const HOSTED_PRODUCT = 'images'

/** The placeholder the old three-step flow filed rows under. */
export const PENDING_PATH = 'pending'

/**
 * Slug + extension for the object name. Byte-for-byte the `safeName` the store
 * flow has always used — changing it would break the legacy path
 * reconstruction above, which is the only reason old backups can be opened at
 * all.
 */
export function safeName(fileName: string | null | undefined): string {
 ...
 * would turn "back up this image" into a thrown TypeError. `getRandomValues`
 * has no such gate, so it is the fallback rather than the other way round.
 */
...
/**
 * Where a hosted image lives:
 * `hosted-uploads/<org_id>/images/<object_id>-<slug>.<ext>`.
 *
 ...
 */
export function hostedImagePath(orgId: string, objectId: string, fileName: string | null | undefined): string {
  return `${orgId}/${HOSTED_PRODUCT}/${objectId}-${safeName(fileName)}`
}
 ...
 */
export function hostedImagePathCandidates(upload: HostedUploadRef): string[] {
  const out: string[] = []
  const recorded = upload.storage_path?.trim()
  if (isUsableStoragePath(recorded, upload.org_id) && recorded) out.push(recorded)
  const legacy = hostedImagePath(upload.org_id, upload.id, upload.file_name)
