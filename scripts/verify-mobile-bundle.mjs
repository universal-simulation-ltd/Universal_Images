...
//
// The fault: Universal Images is hosted at opensource.unisim.co.uk/images/, so the
// production build sets Vite's `base` to `/images/` and index.html asks for
// `/images/assets/index-….js`. Capacitor serves the bundle from `capacitor://localhost`,
// whose document root IS this copied directory — there is no `/images/` inside it, so
